...
import { CourseType, CourseDuration } from '../backend';
import type { 
...
export function useSubmitRegistrationInquiry() {
  ...
  return useMutation({
    mutationFn: async (data: { name: string; phone: string; address: string; course: string; message: string }) => {
      if (!actor) throw new Error('Actor not initialized');
      return actor.submitRegistrationInquiry(data.name, data.phone, data.address, data.course, data.message);
    },
    onSuccess: () => {
