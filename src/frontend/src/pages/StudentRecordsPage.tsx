...
import { CourseDuration, CourseType, type StudentRecord } from "../backend";
import {
  useAddStudentRecord,
...
export default function StudentRecordsPage() {
  ...
  const [editingRecord, setEditingRecord] = useState<StudentRecord | null>(
    null,
  );
  const [formData, setFormData] = useState<StudentFormData>(emptyForm);
  const [reportFilter, setReportFilter] = useState<"all" | "daily" | "monthly">(
    "all",
  );
  ...
  const FormFields = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {(
        ...
        <div key={field.name}>
          <label
            htmlFor={`srfield-${field.name}`}
            className="block text-xs font-medium text-gray-600 mb-1"
          >
            {isBn ? field.labelBn : field.labelEn}
          </label>
          <input
            id={`srfield-${field.name}`}
            type={field.type}
      ...
      <div>
        <label
          htmlFor="srfield-courseType"
          className="block text-xs font-medium text-gray-600 mb-1"
        >
          {isBn ? "কোর্সের ধরন" : "Course Type"}
        </label>
        <select
          id="srfield-courseType"
        ...
        >
          ...
          <option value={CourseType.motorcycle}>
            {isBn ? "মোটরসাইকেল" : "Motorcycle"}
          </option>
        </select>
      </div>
      <div>
        <label
          htmlFor="srfield-courseDuration"
          className="block text-xs font-medium text-gray-600 mb-1"
        >
          {isBn ? "কোর্সের মেয়াদ" : "Course Duration"}
        </label>
        <select
          id="srfield-courseDuration"
          value={formData.courseDuration}
          onChange={(e) =>
            setFormData({
              ...formData,
              courseDuration: e.target.value as CourseDuration,
            })
          }
          className={inputClass}
        >
          <option value={CourseDuration.thirtyDays}>
            {isBn ? "৩০ দিন" : "30 Days"}
          </option>
          <option value={CourseDuration.sixtyDays}>
            {isBn ? "৬০ দিন" : "60 Days"}
          </option>
        </select>
      </div>
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: "#f8f9fa" }}>
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(135deg, #e8f0fe 0%, #f0f7ff 60%, #ffffff 100%)",
        }}
      >
